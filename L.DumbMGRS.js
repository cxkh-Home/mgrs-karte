...
const GZD = L.LayerGroup.extend({
  ...
  onAdd(map) {
    ...
    this._map.on(`viewreset ${this.options.redraw}`, this.getInBoundsGZDs, this);
  },

  onRemove(map) {
    map.off(`viewreset ${this.options.redraw}`, this.getInBoundsGZDs, this);
    this.clearLayers();
  },
...
const MGRS100K = L.LayerGroup.extend({
  ...
  onAdd(map) {
    ...
    this.getVizGrids();
    //! Cannot use 'move' event or it will freeze the entire app
    this._map.on(`viewreset ${this.options.redraw}`, this.getVizGrids, this);
  },

  onRemove(map) {
    map.off(`viewreset ${this.options.redraw}`, this.getVizGrids, this);
    this.clearLayers();
  },
...
const MGRS100Meters = L.LayerGroup.extend({
  options: {
    ...
    maxZoom: 18,
    minZoom: 15,
    gridLetterStyle: 'color: black; font-size:10px;',
    lineStyle: {
      color: 'gray',
      weight: 1,
      opacity: 0.5,
...
// *********************************************************************************** //
// * Leaflet.DumbMGRS - Attach to L object                                           * //
// *********************************************************************************** //
L.GZD = GZD;
L.MGRS100K = MGRS100K;
...
L.mgrs100Meters = function(options) {
    return new L.MGRS100Meters(options);
};
